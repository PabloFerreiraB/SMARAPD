 ...
 *
 * @description
 * Uma Diretiva que recebe configurações definidas pelo desenvolvedor e
 * renderiza uma grid paginada com um campo de pesquisa, inclusa coluna de ações
 * personalizadas
 * 
 * @authors Vinicio Santos e Caique Ferreira
 *
 * @element EA
 *
 */
function functionDiretive() {
    ...
    function fieldLink(scope, el, attrs) {
        ...
        scope.config.getData().then(function (data) {

            if (data && data.length > 0) {

                scope.config.source = data.map(function (item) {
                    return {
                        data: item,
            ...
            scope.bkpList = angular.copy(scope.config.source);

            // Hook in our watched items
            scope.$watchCollection('[page,pageSize,total,disabled]', function () {
                if (scope.config.source[0].data)
                    scope.total = scope.total ? scope.total : scope.config.source.length;
            ...
            var textFilter = sessionStorage.getItem(getName());
            if (textFilter) {
        ...
        function getName() {
            return 'SMARAPD-Componentes.' + scope.config.name
        }

        scope.applyFilter = function () {

            sessionStorage.setItem(getName(), scope.textFilter);

            var resultFilter = scope.bkpList.filter((item) => {
                return item.search.toLowerCase().indexOf(scope.textFilter.toLowerCase()) != -1;
            });

            scope.config.source = resultFilter;

            scope.total = resultFilter[0].data != undefined ? resultFilter.length : 0;
        ...
        scope.limparCampo = function () {
            ...
            scope.config.source = scope.bkpList;
            scope.textFilter = '';
            scope.applyFilter();
        }
    ...
    function paginate(scope) {
        if (scope.config.source[0].data)
            scope.exibitionList = scope.config.source.slice((scope.page - 1) * scope.comboPageSelected, (scope.page * scope.comboPageSelected));
    }
    ...
    function setScopeValues(scope, attrs) {
        ...
        scope.textFirst = scope.textFirst || 'Primeiro';
        scope.textLast = scope.textLast || 'Último';
        scope.textNext = scope.textNext || 'Próximo';
        scope.textPrev = scope.textPrev || 'Anterior';
        ...
        scope.textTitleFirst = scope.textTitleFirst || 'Primeira página';
        scope.textTitleLast = scope.textTitleLast || 'Última página';
        scope.textTitleNext = scope.textTitleNext || 'Próximo';
        scope.textTitlePrev = scope.textTitlePrev || 'Anterior';

        scope.comboPage = scope.comboPage || [10, 30, 50];
        scope.semRegistro = "Nenhum registro encontrado."
    ...
    function build(scope, attrs) {
        ...
        var pageCount = scope.comboPageSelected ? Math.ceil(scope.total / scope.comboPageSelected) : Math.ceil(scope.total / scope.pageSize);

        scope.pageCountExibition = pageCount;
...
angular.module('bw.paging', []).directive('grid', functionDiretive).controller('ctrl',
    ctrl);